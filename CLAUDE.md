...
At the beginning of a task, classify findings into three buckets:
...
Not part of the requested objective, but must be addressed because it prevents investigation, implementation, or validation.

### Backlog

Unrelated cleanup, stale tests, lint warnings, refactors, naming improvements, dependency updates, security advisories, or technical debt.

Report backlog findings once, then leave them unchanged unless the user explicitly expands scope.

Do not fix backlog items merely because they are easy. Do not mix unrelated cleanup into the task branch or PR.
