// Instagram OAuth popup callback. Loaded as an external script (NOT inline)
// so it runs under the site CSP `script-src 'self'` — an inline <script>
// here is silently blocked and the popup hangs on "Connecting…" forever.
(function () {
  var params = new URLSearchParams(window.location.search);
  var code = params.get("code");
  var state = params.get("state");
  var error = params.get("error");
  var errorReason = params.get("error_reason") || params.get("error_description") || error;
  ...
  function notifyParent(payload) {
    ...
    try {
      window.opener.postMessage(
        Object.assign({ type: "rf_ig_oauth" }, payload),
  ...
  if (!code || !state) {
    setStatus("Missing OAuth response", "The Instagram callback didn't include the required parameters.", true);
